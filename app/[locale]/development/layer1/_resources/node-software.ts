import { BitcoinDevResources } from "@/types";

export const nodeSoftwareEn: BitcoinDevResources["node-software"] = [
  {
    name: "Bitcoin Core",
    description:
      "Reference implementation of the Bitcoin protocol providing a robust set of developer tools.",
    url: "https://bitcoin.org/en/bitcoin-core",
    logo: "https://avatars.githubusercontent.com/u/13464320?s=200&v=4",
  },
  {
    name: "btcd",
    description:
      "Alternative full node implementation in Go, designed to be compatible with Bitcoin Core.",
    url: "https://github.com/btcsuite/btcd",
    logo: "https://avatars.githubusercontent.com/u/10235229?s=200&v=4",
  },
  {
    name: "libbitcoin",
    description:
      "Set of cross-platform C++ libraries for building Bitcoin applications.",
    url: "https://github.com/libbitcoin/libbitcoin-system",
    logo: "https://avatars.githubusercontent.com/u/4434452?s=200&v=4",
  },
  {
    name: "bcoin",
    description: "Bitcoin client written in JavaScript by Purse.io.",
    url: "https://github.com/bcoin-org/bcoin",
    logo: "https://avatars.githubusercontent.com/u/17676391?s=200&v=4",
  },
  {
    name: "Bitcoin Knots",
    description:
      "Derivative of Bitcoin Core that includes a variety of enhancements and additional features.",
    url: "https://bitcoinknots.org",
    logo: "https://pbs.twimg.com/profile_images/1964051277920530432/0fGlRT1R_400x400.jpg",
  },
  {
    name: "Node Launcher",
    description:
      "User-friendly desktop application designed to simplify the process of running a Bitcoin and Lightning Network node.",
    url: "https://github.com/lightning-power-users/node-launcher",
    logo: "https://avatars.githubusercontent.com/u/47250091?s=200&v=4",
  },
  {
    name: "Floresta",
    description:
      "Lightweight fully-validating Bitcoin node powered by Utreexo, which includes an integrated Electrum Server.",
    url: "https://github.com/Davidson-Souza/Floresta",
    logo: "https://placehold.co/80x80?text=Floresta",
  },
];

export const nodeSoftwareKo: BitcoinDevResources["node-software"] = [
  {
    name: "Bitcoin Core",
    description:
      "강력한 개발 도구 세트를 제공하는 비트코인 프로토콜의 참조 구현체.",
    url: "https://bitcoin.org/en/bitcoin-core",
    logo: "https://avatars.githubusercontent.com/u/13464320?s=200&v=4",
  },
  {
    name: "btcd",
    description:
      "비트코인 코어와 호환되도록 설계된 Go 언어의 대안 풀 노드 구현체.",
    url: "https://github.com/btcsuite/btcd",
    logo: "https://avatars.githubusercontent.com/u/10235229?s=200&v=4",
  },
  {
    name: "libbitcoin",
    description:
      "비트코인 애플리케이션 구축을 위한 크로스 플랫폼 C++ 라이브러리 세트.",
    url: "https://github.com/libbitcoin/libbitcoin-system",
    logo: "https://avatars.githubusercontent.com/u/4434452?s=200&v=4",
  },
  {
    name: "bcoin",
    description: "Purse.io에서 JavaScript로 작성한 비트코인 클라이언트.",
    url: "https://github.com/bcoin-org/bcoin",
    logo: "https://avatars.githubusercontent.com/u/17676391?s=200&v=4",
  },
  {
    name: "Bitcoin Knots",
    description:
      "다양한 개선사항과 추가 기능을 포함하는 비트코인 코어의 파생 버전.",
    url: "https://bitcoinknots.org",
    logo: "https://pbs.twimg.com/profile_images/1964051277920530432/0fGlRT1R_400x400.jpg",
  },
  {
    name: "Node Launcher",
    description:
      "비트코인과 라이트닝 네트워크 노드 실행 프로세스를 간소화하도록 설계된 사용자 친화적 데스크톱 애플리케이션.",
    url: "https://github.com/lightning-power-users/node-launcher",
    logo: "https://avatars.githubusercontent.com/u/47250091?s=200&v=4",
  },
  {
    name: "Floresta",
    description:
      "통합된 Electrum 서버를 포함하는 Utreexo 기반의 경량 완전 검증 비트코인 노드.",
    url: "https://github.com/Davidson-Souza/Floresta",
    logo: "https://placehold.co/80x80?text=Floresta",
  },
];
